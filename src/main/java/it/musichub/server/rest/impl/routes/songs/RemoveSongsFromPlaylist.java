...
public class RemoveSongsFromPlaylist extends AbstractRoute {

	@PATCH
	@ApiOperation(value = "Remove songs from Playlist", nickname = "RemoveSongsFromPlaylist", tags = "songs")
	@ApiImplicitParams({ //
...
//			@ApiResponse(code = 404, message = "User not found", response = ApiError.class) //
	...
	public Object handle(@ApiParam(hidden = true) Request request, @ApiParam(hidden = true) Response response) throws Exception {
		...
		getUpnpControllerService().getRendererState().getPlaylist().removeSongs(songs);
